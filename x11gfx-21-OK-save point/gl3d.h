...
// Rendering Function
void gl3d_draw_cube(FastCanvas *canvas, Cube3D *cube, float fov, float view_distance);
...
static inline Mat4 gl3d_mat4_rotation(float rx, float ry, float rz) {
    ...
    Mat4 res = {{{0}}};
    // Simple multiplication implementation for internal use
...
// Helper: Rasterize a filled triangle using edge equations / bounding boxes on FastCanvas
...
// Draw Cube Implementation with Backface Culling & Painter's Algorithm Sorting
void gl3d_draw_cube(FastCanvas *canvas, Cube3D *cube, float fov, float view_distance) {
    float hx = cube->size.x / 2.0f;
    float hy = cube->size.y / 2.0f;
    float hz = cube->size.z / 2.0f;

    // 8 Local Vertices of a Cube
    Vec3 local_verts[8] = {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        transformed_verts[i].z += view_distance; // Push away from camera
    }

    // 6 Faces, each made of 2 Triangles (12 triangles total)
    // Indices layout for faces: +X, -X, +Y, -Y, +Z, -Z
    ...
    for (int f = 0; f < 6; f++) {
        ...
        // Backface culling check using surface normal via cross product
        Vec3 edge1 = gl3d_vec3_sub(p1, p0);
        Vec3 edge2 = gl3d_vec3_sub(p2, p0);
        Vec3 normal = gl3d_vec3_cross(edge1, edge2);

        // Check against camera view vector (approximate facing condition)
        if (normal.z < 0) {
            // Triangle 1 (p0, p1, p2)
    ...
    // Simple Bubble Sort for Painter's Algorithm (Furthest triangles drawn first)
    ...
    for (int i = 0; i < tri_count; i++) {
        ...
        int skip = 0;
        for (int k = 0; k < 3; k++) {
            if (triangles[i].p[k].z <= 0.1f) {
                skip = 1;
                break;
            }
            float factor = fov / triangles[i].p[k].z;
            proj[k].x = triangles[i].p[k].x * factor + cx;
            proj[k].y = -triangles[i].p[k].y * factor + cy; // Invert Y for screen space
